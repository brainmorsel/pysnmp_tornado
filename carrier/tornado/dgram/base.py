...
class DgramSocketTransport(AbstractSocketTransport):
    ...
    def sendMessage(self, outgoingMessage, transportAddress):
        ...
        debug.logger & debug.flagIO and print('sendMessage: outgoingMessage queued (%d octets) %s' % (len(outgoingMessage), debug.hexdump(outgoingMessage)))
    ...
    def handle_write(self):
        if not self.__outQueue: return
        outgoingMessage, transportAddress = self.__outQueue.pop(0)
        debug.logger & debug.flagIO and print('handle_write: transportAddress %r -> %r outgoingMessage (%d octets) %s' % (self.__getsockname(), transportAddress, len(outgoingMessage), debug.hexdump(outgoingMessage)))
        if not transportAddress:
            debug.logger & debug.flagIO and print('handle_write: missing dst address, loosing outgoing msg')
            return
        try:
            self.socket.sendto(outgoingMessage, transportAddress)
        except socket.error:
            if sys.exc_info()[1].args[0] in sockErrors:
                debug.logger & debug.flagIO and print('handle_write: ignoring socket error %s' % (sys.exc_info()[1],))
            else:
    ...
    def handle_read(self):
        try:
            ...
            debug.logger & debug.flagIO and print('handle_read: transportAddress %r -> %r incomingMessage (%d octets) %s' % (transportAddress, self.__getsockname(), len(incomingMessage), debug.hexdump(incomingMessage)))
            ...
            if sys.exc_info()[1].args[0] in sockErrors:
                debug.logger & debug.flagIO and print('handle_read: known socket error %s' % (sys.exc_info()[1],))
